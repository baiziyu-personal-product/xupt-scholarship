...
func setProcessStepStartTemplate(e *email.Email) {
	e.Subject = time.Now().Format("2006") + "年研究生奖学金评定流程的通知"
	...
	e.HTML = []byte(`
      ...
      <div style="width: 100%; height: 100px; padding-top: 3px;">
        <img
          ...
          src="https://www.xiyou.edu.cn/newWeb/images/xy_logo.png"
        />
      </div>
      <div style="background-color: #fff; padding-top: 20px;">
        ...
        >
          <p style="text-indent: 2em;">
            本次研究生奖学金评定流程将于` + currentDate + `开始` + currentDate + `流程，请尚未当前` + currentDate + `流程的同学以及管理员抓紧完善当前流程工作内容。
          </p>
          <p style="text-indent: 2em;">
...
func setProcessOpenTemplate(e *email.Email) {
	e.Subject = "开启" + time.Now().Format("2006") + "年研究生奖学金评定流程的通知"
	currentDate := time.Now().Format("2006-01-02")
	e.HTML = []byte(`
	<div style="background-color: #014a95;">
      <div style="width:100%;height: 100px;padding-top: 3px">
          <img style="width: 398px;height: 94px;" src="http://www.xiyou.edu.cn/newWeb/images/xy_logo.png" />
      </div>
      <div style="background-color: #fff;padding-top: 20px;">
          <h2 style="text-align:center;">《西安邮电大学研究生奖学金流程开始的通知》</h1>
            <p style="text-align: center;font-size: 13px;color: #aaa;">` + currentDate + `</p>
            <div style="font-size: 16px;margin:20px;border-radius: 4px;box-shadow: 1px 1px 2px #ccc;padding: 20px 40px;border: 1px solid rgb(240, 239, 239);">
              <p style="text-indent: 2em;">本次奖学金评定流程将于` + currentDate + `开始，您已经被添加为本次评定流程的管理人员，负责参与奖学金的评定。</p>
...
func SendEmail(receiveEmailList []string) {
	...
	for _, receiver := range receiveEmailList {
		...
		setProcessOpenTemplate(e)
		//setProcessStepStartTemplate(e)
		ch <- e
	}
